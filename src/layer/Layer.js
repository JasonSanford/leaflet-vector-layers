            ...
            for (var i = 0; i < data.features.length; i++) {
                ...
                if (this.options.uniqueField) {
                    ...
                    for (var i2 = 0; i2 < this._vectors.length; i2++) {
                        ...
                        if (feature.properties[this.options.uniqueField] == this._vectors[i2].properties[this.options.uniqueField]) {
                            ...
                            if (this.options.dynamic) {
                                ...
                                if (this._getGeometryChanged(this._vectors[i2].geometry, feature.geometry)) {
                                    ...
                                    if (!isNaN(feature.geometry.coordinates[0]) && !isNaN(feature.geometry.coordinates[1])) {
                                        this._vectors[i2].geometry = feature.geometry;
                                ...
                                var propertiesChanged = this._getPropertiesChanged(this._vectors[i2].properties, feature.properties);
                                
                                if (propertiesChanged) {
                                    var symbologyPropertyChanged = this._getPropertyChanged(this._vectors[i2].properties, feature.properties, this.options.symbology.property);
                                    this._vectors[i2].properties = feature.properties;
                ...
                if (!onMap || !this.options.uniqueField) {
                    ...
                    // Convert GeoJSON to Leaflet vector (Point, Polyline, Polygon)
                        var vector_or_vectors = this._geoJsonGeometryToLeaflet(feature.geometry, this._getFeatureVectorOptions(feature));
                        feature[vector_or_vectors instanceof Array ? "vectors" : "vector"] = vector_or_vectors;
                    } else if (this instanceof lvector.EsriJSONLayer) {
                        // Convert Esri JSON to Google Maps vector (Point, Polyline, Polygon)
                        var vector_or_vectors = this._esriJsonGeometryToLeaflet(feature.geometry, this._getFeatureVectorOptions(feature));
                        feature[vector_or_vectors instanceof Array ? "vectors" : "vector"] = vector_or_vectors;
                    }
                    
                    // Show the vector or vectors on the map
                    if (feature.vector) {
                        this.options.map.addLayer(feature.vector);
                    } else if (feature.vectors && feature.vectors.length) {
                        for (var i3 = 0; i3 < feature.vectors.length; i3++) {
                            this.options.map.addLayer(feature.vectors[i3]);
                        }
                    }
                    
                    // Store the vector in an array so we can remove it later
                    this._vectors.push(feature);

                    var me = this;

                    (function(feature) {
                        if (me.options.events) {
                            var eventTypes = ["mouseover", "mouseout", "click"];
                            for (var type in eventTypes) {
                                (function(type) {
                                    if (me.options.events[eventTypes[type]]) {
                                        if (feature.vector) {
                                            feature.vector.on(eventTypes[type], function(event) {
                                                me.options.events[eventTypes[type]](event, feature.vector, feature.properties);
                                            });
                                        } else if (feature.vectors) {
                                            for (var i3 = 0; i3 < feature.vectors.length; i3++) {
                                                (function(it) {
                                                    feature.vectors[it].on(eventTypes[type], function(event) {
                                                        me.options.events[eventTypes[type]](event, feature.vectors[it], feature.properties);
                                                    });
                                                }(i3));
                                            }
                                        }
                        ...
                        if (me.options.popupTemplate) {
                            
                            me._setPopupContent(feature);
